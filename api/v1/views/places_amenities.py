#!/usr/bin/python3
'''includes the API's places_amenities view.
'''
...
def handle_places_amenities(place_id=None, amenity_id=None):
    '''the locations endpoint's method handler.
    '''
    handlers = {
...
def get_place_amenities(place_id=None, amenity_id=None):
    '''obtains a location's amenities using the provided ID.
    '''
    if place_id:
...
def remove_place_amenity(place_id=None, amenity_id=None):
    '''removes an amenity from a location with a given id if it has one.
    '''
...
def add_place_amenity(place_id=None, amenity_id=None):
    '''adds a facility with a specified id to a location with a specified id.
    '''
    if place_id and amenity_id:
        place = storage.get(Place, place_id)
        if not place:
